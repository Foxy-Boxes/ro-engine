...
#include <math.h>
    ...
    Matrix44& Matrix44::initProjection(Window* w,double near,double far,double fov){
        double aRatio = (double)w -> getHeight() / (double)w -> getWidth();
        double fovCalc = 1.0 / tan(fov * 0.5 / 180.0 * 3.14159265359);
        m[0][0] = aRatio*fovCalc; m[0][1] = 0.0; m[0][2] = 0; m[0][3] = 0.0; 
        m[1][0] = 0.0; m[1][1] = fovCalc; m[1][2] = 0.0; m[1][3] = 0.0; 
        m[2][0] = 0.0; m[2][1] = 0.0; m[2][2] = far / (far - near); m[2][3] = 1.0;
        m[3][0] = 0.0; m[3][1] = 0.0; m[3][2] = (-far * near) / (far - near); m[3][3] = 0.0;
        return *this;
    }
